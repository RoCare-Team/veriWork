...
import {
  employeeKeys,
  fetchEmployeeSmtpSettings,
  updateEmployeeSmtpSettings,
  sendEmployeeSmtpTest,
} from '../../api/employee'
...
function SmtpSettingsSection() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: employeeKeys.smtpSettings,
    queryFn: fetchEmployeeSmtpSettings,
  })
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      queryClient.setQueryData(employeeKeys.smtpSettings, res)
      toast(
        s.configured
          ? 'Email settings saved'
          : 'Email settings saved (incomplete — add a password to enable sending)',
        'success',
      )
    },
    onError: (err) => toast(err.message || 'Failed to save email settings', 'error'),
  })

  const testMutation = useMutation({
    mutationFn: () => sendEmployeeSmtpTest({}),
    onSuccess: (res) => {
      const r = unwrap(res)
      toast(
        r.sent ? `Test email sent to ${r.to}` : 'Test not sent — check configuration',
        r.sent ? 'success' : 'error',
      )
    },
    onError: (err) => toast(err.message || 'Email test failed', 'error'),
  })

  if (isLoading) {
    return (
      <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm md:p-6">
        <h3 className="m-0 text-base font-bold text-slate-900">Email (SMTP) Settings</h3>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="m-0 mt-1 text-xs text-slate-500">
            Configure your mailbox once — when you request verification of a past job, the request
            email is sent to the HR from your own address.
          </p>
        </div>
        <span
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              placeholder={hasPassword ? '•••••••• (unchanged)' : 'SMTP password / app password'}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              placeholder={settings.defaultSenderEmail || 'you@gmail.com'}
              className={inputClass}
            />
            <p className="mt-1 text-[11px] text-slate-400">
              Defaults to your registered email
              {settings.defaultSenderEmail ? ` (${settings.defaultSenderEmail})` : ''}.
            </p>
          </div>
        </div>
        ...
        <label className="flex items-center gap-2 text-sm">
          ...
          <span className="font-semibold text-slate-700">
            Use SSL/TLS (secure connection, usually port 465)
          </span>
        </label>
        ...
        {!settings.configured && (
          <p className="m-0 text-[11px] text-slate-400">
            Save a complete configuration (host, username, password, sender email) to enable test
            emails and live sending.
          </p>
        )}
      </form>
    </section>
