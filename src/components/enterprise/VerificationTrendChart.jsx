      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="h-auto w-full" role="img" aria-label="Trend line chart">
        <defs>
          <linearGradient id="dash-trend-fill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#1a3a8f" stopOpacity="0.18" />
        ...
        {points.map((p, i) => (
          <g key={`${axisLabels[i]}-${i}`}>
            ...
            <circle cx={p.x} cy={p.y} r="7" fill="#1a3a8f" fillOpacity="0.12" />
            <circle cx={p.x} cy={p.y} r="5" fill="#1a3a8f" stroke="#fff" strokeWidth="2.5" />
          </g>
        ))}
      </svg>
